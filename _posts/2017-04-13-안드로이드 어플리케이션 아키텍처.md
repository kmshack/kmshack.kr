...
Android 개발 생태계는 빠르게 변하며, 앱 아키텍처 역시 도구와 요구사항에 맞춰 발전해 왔다. 이 글은 [ribot 팀](http://ribot.co.uk/us)이 AsyncTask 중심 구조에서 RxJava와 MVP 기반 구조로 옮겨 간 과정과 그 과정에서 얻은 교훈을 정리한다.  
...
```
Activity 또는 Fragment과 같은 View Layer의 구성 요소는 이 메서드를 호출하고 리턴된 Observable를 통해 RecyclerView에 직접 표시할 수 있습니다.
...
- DataManager는 이전에 View Layer의 일부였던 책임을 담당합니다. 따라서 Activity와 Fragment를 더욱 가볍게 만듭니다.
- Activity, Fragment에서 DataManager, Helper로 코드를 이동하면 유닛 테스트 작성이 쉬워집니다.
...
- Activity 또는 Fragment와 같은 View Layer의 구성 요소가 더 가벼워졌지만 RxJava Subscriptions 관리, 에러 탐지 등과 관련하여 상당한 양의 작업을 처리해야 합니다.  
